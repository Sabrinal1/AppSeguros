...
import java.util.ArrayList;
	...
	public class Aseguradora implements Mostrable {
		...
		public Auto generarSeguroAuto(float valorAsegurado, MarcaAuto marca, String modelo) {
			return new Auto(valorAsegurado, marca, modelo);
		}

		public Celular generarSeguroCelular(float valorAsegurado, String marca, String modelo) {
		...
		public void mostrar() {
			...
			System.out.println(MSG_ESTADISTICA_AUTOS_MASC + e.getAseguradosAutoMasculino());
			System.out.println(MSG_ESTADISTICA_EDAD_CEL + e.getPromedioEdadTenenciaCelular());

			System.out.println(MSG_ESTADISTICA_PREMIO_AUTO + e.getPremioMenorAuto());
		}
		...
		private Estadistica procesarEstadisticas() {
			...
			return new Estadistica(aseguradosAutoMasculino,
					cantCelular > 0 ? (float) sumEdadCelular / (float) cantCelular : 0, premioMenorAuto);
		}

		@Override
		public String texto() {
			return TXT_ENCABEZADO + nombreAseguradora;
		}
		...
		private class Estadistica {
			...
			public Estadistica(int aseguradosAutoMasculino, float promedioEdadTenenciaCelular, float premioMenorAuto) {
